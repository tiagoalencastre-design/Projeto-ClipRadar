...
# Validade do link de confirmação. 24h é o costume: tempo de sobra para a
# pessoa ver o e-mail, e curto o bastante para um link vazado (caixa de
# entrada comprometida, histórico de navegador) parar de servir.
VERIFICATION_TOKEN_HOURS = 24


def verify_email_token(token: str) -> bool:
    """
    Confirma o e-mail se o token existir E ainda estiver dentro do prazo.

    O campo verification_sent_at já era gravado, mas ninguém o consultava:
    na prática, o link nunca expirava.
    """
    if not token:
        return False
    ...
    with get_db() as conn:
        row = conn.execute(
            "SELECT id, verification_sent_at FROM users WHERE verification_token = ?",
            (token,),
        ).fetchone()
        if not row:
            return False

        if _token_expired(row["verification_sent_at"]):
            # Não apagamos o token: o usuário pode pedir um novo pelo
            # "reenviar confirmação", e apagar aqui deixaria a conta num
            # estado sem token e sem verificação.
            return False
...
def _token_expired(sent_at: str | None) -> bool:
    """Data ilegível conta como expirada — na dúvida, não confirma."""
    if not sent_at:
        return True
    try:
    ...
    return datetime.now(timezone.utc) - sent > timedelta(hours=VERIFICATION_TOKEN_HOURS)
